...
resource "aws_iam_role" "prod-region1-role" {
    name = "prod-region1-role"
...
resource "aws_iam_role_policy_attachment" "prod-ec2_full-access" {
    role = aws_iam_role.prod-region1-role.name
    policy_arn = "arn:aws:iam::aws:policy/AmazonEC2FullAccess"
}

resource "aws_iam_role_policy_attachment" "prod-s3-access" {
    role = aws_iam_role.prod-region1-role.name
    policy_arn = "arn:aws:iam::aws:policy/AmazonS3ReadOnlyAccess" 
}

resource "aws_iam_instance_profile" "prod-ec2-profile" {
...
resource "aws_launch_template" "pord-template" {
    name = "prod-template"
